...
public class GUICircuitBuilder extends GuiContainer {
	...
	private String hashStr = "[ BCB Not Found ]";
		...
		switch (guibutton.id) {
			case 1:
				if (cbuilder.ready == true) {
					cbuilder.installed = true;
					hashStr = "[ BCB Installed ]";
					if (mc.thePlayer.getHeldItem() != null && mc.thePlayer.getHeldItem().isItemEqual(cb)) {
						ItemStack c = (new ItemStack(PlagueItems.itemCircuitParts, 0, 5));
						c.stackSize--;
						c.setItemName("sucked");
					}
				}
		}
	}
	...
	private boolean checkForCircuitBoard() {
		...
		if (mc.thePlayer.getHeldItem() != null && mc.thePlayer.getHeldItem().isItemEqual(cb)) {
			hashStr = "[ BCB Found: Ready To Install ]";
			cbuilder.ready = true;
