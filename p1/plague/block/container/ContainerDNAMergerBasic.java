...
import p1.plague.block.container.slot.SlotDNAMerger;
...
public class ContainerDNAMergerBasic extends Container {
	...
	public ContainerDNAMergerBasic(InventoryPlayer inventoryplayer,
			TileEntityDNAMergerBasic tileentityMerger) {
		...
		/** output old slot 3 **/
		this.addSlotToContainer(new SlotDNAMerger(inventoryplayer.player,
				tileentityMerger, 2, 122, 33));
		/** power old slot 4 **/
		...
		for (i = 0; i < 3; ++i) {
			for (int j = 0; j < 9; ++j) {
				this.addSlotToContainer(new Slot(inventoryplayer,
						j + i * 9 + 9, 24 + j * 18, 166 + i * 18));
			}
		}
		/** hotbar **/
		for (i = 0; i < 9; ++i) {
			/**
			 * 24 is LEFT TO RIGHT postioning... 18 is spacing... 255 is UP TO
			 * DOWN positioning
			 **/
			this.addSlotToContainer(new Slot(inventoryplayer, i, 24 + i * 18,
					255 - 30));
		}
	}
	...
	public void addCraftingToCrafters(ICrafting par1ICrafting) {
		...
		par1ICrafting
				.sendProgressBarUpdate(this, 0, this.merger.mergerCookTime);
