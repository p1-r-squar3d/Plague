...
import p1.plague.block.BlockSyringeScannerBasic;
import p1.plague.gui.GUISyringeScannerBasic;
import p1.plague.item.PlagueItems;
import p1.plague.recipe.SyringeScannerRecipes;
import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;

public class TileEntitySyringeScannerBasic extends TileEntity implements
		ISidedInventory {
	...
	public void updateEntity() {
		...
		if (!this.worldObj.isRemote) {
			...
			if (this.power < this.maxPower
					&& this.getItemPower(this.slots[1]) > 0) {
				...
				if (!this.slots[1].isItemStackDamageable()) {
					...
					this.power += getItemPower(this.slots[1]);
					this.slots[1] = new ItemStack(this.slots[1].getItem(),
							this.slots[1].stackSize,
							this.slots[1].getItemDamage() + 1);

					if (this.slots[1].getItemDamage() >= this.slots[1]
							.getMaxDamage()) {
							this.slots[1].stackSize--;
						if (this.slots[1].stackSize == 0) {
							this.slots[1] = null;
			...
			if (flag != this.power > 0) {
				flag1 = true;
				BlockSyringeScannerBasic.updateFurnaceBlockState(
						this.power > 0, this.worldObj, this.xCoord,
	...
	private boolean canSmelt() {
		...
		ItemStack itemstack = SyringeScannerRecipes.getSmeltingResult(slots[0]
				.getItem().itemID);
		...
		if (slots[6].stackSize < getInventoryStackLimit()
	...
	public void smeltItem() {
		if (this.canSmelt()) {
			ItemStack itemstack = SyringeScannerRecipes
					.getSmeltingResult(this.slots[0].itemID);
	...
	public static int getItemPower(ItemStack par0ItemStack) {
		if (par0ItemStack == null) {
			...
			/**
			 * if (i == Item.redstone.itemID) return 10; if (i ==
			 * Item.glowstone.itemID) return 11; if (i == Item.diamond.itemID)
			 * return 10000;
			 **/
			if (i == PlagueItems.itemBattery.itemID)
				return 1;
			return 0;
		}
	}
	...
	public static int getUpgradeItem(ItemStack par0ItemStack) {
		if (par0ItemStack == null) {
			...
			if (i == Item.diamond.itemID)
				return 100;
			if (i == Item.ingotGold.itemID)
				return 300;
			return 0;
		}
	}
